...
def print_failed_structures(failures, outfile=sys.stdout):
    if failures:
        print('The algorithm has failed in the following structures:', file=outfile)
        print(file=outfile)
        print('%30s%8s        %s' % \
...
def print_scores(level, model_scores, outfile=sys.stdout):
    print('The following are the top-scoring structures for level %d:' % level, file=outfile)
    print(file=outfile)
    print('%30s%10s%10s%13s%13s%13s%10s%10s' % \
          ('structure', 'row', 'col', 'total', 'row impvt.', 'col impvt.', 'z (row)', 'z (col)'), file=outfile)
    print(file=outfile)
    for ms in model_scores:
        print('%30s%10.2f%10.2f%13.2f%13.2f%13.2f%10.2f%10.2f' % \
              (grammar.pretty_print(ms.structure), ms.row_score, ms.col_score, ms.total,
               ms.row_improvement, ms.col_improvement, ms.z_score_row, ms.z_score_col), file=outfile)
    print(file=outfile)
    print(file=outfile)
    

def print_model_sequence(model_scores, outfile=sys.stdout):
    print("Here are the best-performing structures in each level of the search:", file=outfile)
    print(file=outfile)
    print('%10s%25s%13s%13s%10s%10s' % \
          ('level', 'structure', 'row impvt.', 'col impvt.', 'z (row)', 'z (col)'), file=outfile)
    print(file=outfile)
    for i, ms in enumerate(model_scores):
        print('%10d%25s%13.2f%13.2f%10.2f%10.2f' % \
              (i+1, grammar.pretty_print(ms.structure), ms.row_improvement, ms.col_improvement,
               ms.z_score_row, ms.z_score_col), file=outfile)
    print(file=outfile)
    print(file=outfile)
...
def print_running_times(running_times, outfile=sys.stdout):
    ...
    print('Total CPU time was %s. Here is the breakdown:' % format_time(total), file=outfile)
    print(file=outfile)
    print('%30s%8s        %s' % \
          ('structure', 'level', 'time'), file=outfile)
    print(file=outfile)
    ...
    for rt in running_times:
        ...
        print('%30s%8d        %s' % (grammar.pretty_print(rt.structure), rt.level, time_str), file=outfile)
    print(file=outfile)
    print(file=outfile)
...
def print_learned_structures(results, outfile=sys.stdout):
    ...
    print(file=outfile)
    for r in results:
        print('%25s%25s' % (r.expt_name, grammar.pretty_print(r.structure)), file=outfile)
    print(file=outfile)
    print(file=outfile)
