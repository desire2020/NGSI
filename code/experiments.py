...
class LargeParams(DefaultParams):
    ...
    num_splits = 2
    num_samples = 2
    max_dim_predictive = 128         # use a subset of the dimensions to save time

class FineParams(DefaultParams):
...
def random_shrink(t, width=200):
    if t.shape[0] <= width:
        return t
    idx = np.array(list(range(t.shape[0])))
